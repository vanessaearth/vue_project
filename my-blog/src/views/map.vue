...
<template>
  <div class="container">
    ...
    <LMap ref="myMap"
      ...
      <LTileLayer :url="url" />
      <LPolyline v-for="(item,index) in polyline"
                  :key="index"
                  :lat-lngs="item.latlngs"
                  :color="item.color"></LPolyline>
      <!-- 起点终点 -->
      <LMarker v-for="(item,index) in markerStartEnd"
                :key="`start${index}`"
                :lat-lng="item.latlng"
      ...
      <!-- 加减速及休息点 -->
      <LMarker v-for="(item,index) in markerLatLng"
                :key="`point${index}`"
                :lat-lng="item.latlng">
        <LTooltip :options="{ permanent: true, interactive: true, direction: 'top',offset:[0,30],className:'tooltip-black' }">
